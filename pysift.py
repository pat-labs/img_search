...
def generateBaseImage(image, sigma, assumed_blur):
    ...
    )  # [What is the resulting σ after applying successive gaussian blur?](https://math.stackexchange.com/questions/3159846/what-is-the-resulting-sigma-after-applying-successive-gaussian-blur)
    return GaussianBlur(
...
def computeNumberOfOctaves(image_shape):
    ...
    - y/(2**x) = 1
    Legend:
    - x: number of times we can halve the base image
    - y: the shorter side length of the image | image should have a side length of at least 1 pixel
    - -1: to have a integer number of layers in out image pyramid
    """
