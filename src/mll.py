...
EFFICIENTNETB0_PATH = "model/efficientnetb0.keras"
...
def fill_model():
    try:
        ...
        print(
            f"Make sure the file '{VGG16_PATH}' exists and is a valid Keras model file."
        )
    except ValueError as e:  # Handle potential issues with custom objects
        print(f"ValueError loading model: {e}")
        print(
            "This might be due to custom layers or functions. If so, you'll need to provide them in a 'custom_objects' dictionary."
        )
...
def get_model_EfficientNetB0(image_shape, train_generator, val_generator):
    ...
    model_checkpoint = ModelCheckpoint(
        "efficientnetb0_best_updated.keras", save_best_only=True
    )

    # Train the model
    history = efficientnet_model1.fit(
        train_generator,
...
def transfer_learning_VGG16(image_shape, class_counts, train_generator, val_generator):
    ...
    base_model = VGG16(
        weights="imagenet", include_top=False, input_shape=image_shape, pooling="max"
    )
    ...
    model_checkpoint = ModelCheckpoint(
        "vgg16_fine_tuned_best.keras", save_best_only=True
    )
    ...
    disp_vgg = ConfusionMatrixDisplay(
        confusion_matrix=cm_vgg, display_labels=val_generator.class_indices.keys()
    )
...
def draw_rectangle_on_detection(img_path, model, last_conv_layer_name, class_indices):
    ...
    pred_class_name = list(class_indices.keys())[
        list(class_indices.values()).index(pred_class_idx)
    ]
    ...
    last_conv_layer_model = tf.keras.Model(
        inputs=model.input, outputs=last_conv_layer.output
    )
    ...
    thresh = np.where(heatmap[:, :, 0] > heatmap[:, :, 0].mean(), 255, 0).astype(
        np.uint8
    )
    ...
    output_filename = os.path.join(
        current_directory_path, "result", f"{pred_class_name}_detection.jpg"
    )
...
def plot_average_height_width(classes, train_dir):
    ...
    for cls, (avg_height, avg_width) in class_averages.items():
        print(
            f"{cls}: Average Height = {avg_height:.2f}, Average Width = {avg_width:.2f}"
        )

    # Create a DataFrame for average dimensions per class
    class_averages_df = pd.DataFrame.from_dict(
        class_averages, orient="index", columns=["Average Height", "Average Width"]
    )
...
def test_model():
    ...
    result_img = draw_rectangle_on_detection(
        img_path, VGG16_model, last_conv_layer_name, class_indices
    )
