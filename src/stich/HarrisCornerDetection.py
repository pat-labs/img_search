...
def gradient_x(img):
    sobel_x = np.array(
        [[-1 / 8, 0, 1 / 8], [-2 / 8, 0, 2 / 8], [-1 / 8, 0, 1 / 8]], dtype=np.float32
    )
...
def gradient_y(img):
    sobel_y = np.array(
        [[1 / 8, 2 / 8, 1 / 8], [0, 0, 0], [-1 / 8, -2 / 8, -1 / 8]], dtype=np.float32
    )
...
def draw_interest_pts(
    original_image,
    harris_response,
    threshold,
    descriptors_interest_points,
    interest_pts,
):
    ...
    for y in range(original_image.shape[1]):
        for x in range(original_image.shape[0]):
            if (int(harris_response[x, y]) > threshold) and local_maximum_neighborhood(
                harris_response, x, y
            ):
...
def find_interest_points(src, thresh_percent, saveCornerImage, savedImagePath):
    ...
    # Harris corner
    harris_corners = draw_interest_pts(
        src, harris_response, thresh, descriptors_interest_points, interest_points
    )
    if saveCornerImage:
